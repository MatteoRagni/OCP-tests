...
module BikeSteering
  ...
  case RUBY_PLATFORM
  when /darwin/
    LIB_PATH = base_path+"/libBikeSteering.dylib"
  when /linux/
    LIB_PATH = base_path+"/libBikeSteering.so"
  when /mingw/
    LIB_PATH = base_path+"/libBikeSteering.dll"
  else
  ...
  class Spline
    ...
    def initialize(type="pchip")
      @id = self.__id__.to_s
      BikeSteering.SPLINE_new @id, type
      return @id
    end

    def clear
      BikeSteering.SPLINE_select @id
      BikeSteering.SPLINE_init
    end

    def build
      BikeSteering.SPLINE_select @id
      BikeSteering.SPLINE_build
    end

    def type
      BikeSteering.SPLINE_select @id
      BikeSteering.SPLINE_get_type_name
    end

    def push_back( x, y )
      BikeSteering.SPLINE_select @id
      BikeSteering.SPLINE_push x, y
    end

    def value( x )
      BikeSteering.SPLINE_select @id
      return BikeSteering.SPLINE_eval(x)
    end

    def D( x )
      BikeSteering.SPLINE_select @id
      return BikeSteering.SPLINE_eval_D(x)
    end

    def DD( x )
      BikeSteering.SPLINE_select @id
      return BikeSteering.SPLINE_eval_DD(x)
    end

    def DDD( x )
      BikeSteering.SPLINE_select @id
      return BikeSteering.SPLINE_eval_DDD(x)
    end
  ...
  class OCPSolver
    ...
    def initialize(data=nil)
      ...
      BikeSteering.GC_select @id+"_result"
      BikeSteering.GC_select @id
      ObjectSpace.define_finalizer(self, proc { BikeSteering.GC_delete @id })
      return @id
    end

    def solve( convert = true )
      BikeSteering.GC_select @id
      BikeSteering.GC_from_hash @data
      ok  = BikeSteering.OCP_setup( @id, BikeSteering.GC_mem_ptr(@id) )
      out = BikeSteering.GC_to_hash
      raise RuntimeError, "Failed to do setup: #{out[:Error]}\n" unless ok
      
      ok = BikeSteering.OCP_solve( @id, BikeSteering.GC_mem_ptr(@id), BikeSteering.GC_mem_ptr(@id+"_result"))
      BikeSteering.GC_select @id+"_result"
      out = BikeSteering.GC_to_hash
      ...
      if convert then
        ...
        return BikeSteering.GC_to_hash
      end
    end

    def write_ocp_solution( fname ) 
      BikeSteering.OCP_write_ocp_solution( @id, fname )
    end
